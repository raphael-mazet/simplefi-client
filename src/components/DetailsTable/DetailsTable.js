...
export default function DetailsTable({txHistory, name}) {
  ...
  function createCellValues(tempValues) {
    ...
    // @dev: pricePerFarmSeedToken is only used when there is a reward claim (txValue based on ppt, balance value based on pprt)
    const {pricePerToken, pricePerFarmSeedToken} = cellValues[4];
    
    cellValues[2] = Number((cellValues[2] * pricePerToken).toFixed(2)).toLocaleString(); //tx value
    
    //set hist. balance value
    if (pricePerFarmSeedToken) {
      cellValues[4] = Number((pricePerFarmSeedToken * balance).toFixed(2)).toLocaleString(); 
    } else {
    ...
    <div className="field-details-tx-table">
      ...
      <div className="details-tx-table-rows">
        {txHistory.map((tx, txIndex) => {
          ...
          // @dev: returns [date, action, amount, effect-on-balance, {pricePerToken, pricePerFarmSeedToken}]
